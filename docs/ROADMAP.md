...
| `check_body_comments.py` | Subprograma **privado** sem comentário — o body é o único lugar onde ele pode ser documentado, e 27 não tinham uma linha. Também pega comentário em inglês ou francês herdado do porte, código comentado (nove trechos, um deles o ramo do `/Dest` que nunca funcionou) e caminho que não existe mais: quando `scripts/` virou `dev/scripts/`, **doze** comentários ficaram apontando para o nada, e comentário quebrado não quebra nada |
