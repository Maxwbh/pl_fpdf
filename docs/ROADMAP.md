...
> **Os numeros de DEMANDA desta secao sao de 8 de setembro de 2026, e nao
> foram remedidos.** Servem para ordenar a fila, nao como medicao corrente:
> quem precisar deles para decidir alguma coisa hoje refaz a contagem antes. O
> que mudou desde entao esta dito item a item — o item 1 saiu na 3.4.0.

> **O ESTADO de cada item foi reconferido no codigo em 13/09/2026**, e e o que
> esta escrito abaixo. O que se olhou, e o que se achou:
>
> | Item | Como se conferiu | Estado |
> |---|---|---|
> | 1. WinAnsi | tabela cp1252 no `.pkb`, recusa `ORA-20203`, `test_winansi.sql` | **concluido** |
> | 2. Subset de TTF | `/FontFile2` sai, e o proprio codigo diz "sem subset: a fonte inteira vai" | ausente, como descrito |
> | 3. Sumario e marcadores | nenhuma ocorrencia de `/Outlines` no gerador | ausente |
> | 4. PDF marcado | nenhuma ocorrencia de `/StructTreeRoot` | ausente, e no backlog |
> | Achado da API | 118 das 119 publicas com chamador | praticamente fechado |
>
> Nenhum item mudou de estado sem estar dito aqui — reconferir e barato, e
> supor que a fila continua valida porque foi verdade em setembro nao e.
...
### Achado proprio: metade da API publica nao tinha chamador — FECHADO

Quando a medicao foi feita, 69 das 138 APIs publicas eram exercitadas por
`examples/` e `dev/tests/`. As outras 69 compilavam e ninguem as chamava neste
repositorio — nao havia como saber se funcionavam. Nao e o mesmo que estarem
quebradas, e e exatamente a situacao em que `AddWatermark` passou meses
marcado como pronto sem desenhar nada.
...
nao depende de recurso externo: **de 48 APIs sem chamador para 6**.

**Medicao de 13/09/2026: 118 das 119 APIs publicas do `PL_FPDF` tem chamador.**
Falta uma, o `getImageFromUrl`, que exige ACL de rede -- e o que ha sobre ele e
o teste da **recusa**, que e o que se pode afirmar sem concessao no banco. As
outras cinco que a nota acima listava foram cobertas pela chamada que afere a
recusa com o erro proprio: `LoadTTFFromFile` sem DIRECTORY, `OutputFile` e
`Output` sem destino, `Image` sem ACL, `AddFont` sem arquivo. O
`check_test_calls.py` guarda o numero no CI.
