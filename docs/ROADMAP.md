...
| Item | Situacao |
|------|----------|
| **Fonte TrueType registrada nao chega ao documento** | **Aberto, achado em setembro/2026 ao escrever o primeiro teste dessas APIs.** `AddTTFFont` e `LoadTTFFromFile` guardam a fonte em `g_ttf_fonts`, um cache **sem consumidor**: o `SetFont` nunca o consulta, e nada emite os bytes no PDF. O embutimento que existe e o do caminho legado — `AddFont` com `FontFiles` e `/Length1` —, que le metricas de ARQUIVO. E o `parse_ttf_header` nao interpreta o arquivo: confere o magic number e **inventa** o resto — `units_per_em := 1000`, `ascent := 800`, `descent := -200` sao literais no codigo, entao um BLOB de 28 bytes sem tabela nenhuma devolve as mesmas metricas de uma fonte real. E o quinto caso desta familia, depois do `AddWatermark` (marcado pronto em fevereiro, so desenhou em agosto) e dos quatro do `Link`: **API que compila, registra e nao entrega**. A spec e a referencia passam a dizer isso onde o chamador le, e ha caso de regressao em `dev/tests/test_api_sem_chamador.sql` que falha se as metricas deixarem de ser constantes — porque ai o parser passou a existir e a documentacao precisa mudar. **Falta decidir:** ligar o `SetFont` ao cache e emitir o `/FontFile2` (que e o caminho da HU-02, com subset), ou recusar o registro enquanto ele nao levar a lugar nenhum. |
