...
> **Página gerada** do Javadoc de `src/PL_FPDF.pks` e `src/PL_FPDF_UTIL.pks`.
...
**PL_FPDF_UTIL — QR, código de barras, compressão e criptografia** — [qr_matriz](#qr_matriz) · [bc_padrao](#bc_padrao) · [inflate](#inflate) · [deflate](#deflate) · [hex_do_byte](#hex_do_byte) · [crypto_md5](#crypto_md5) · [crypto_rc4](#crypto_rc4) · [crypto_rc4_blob](#crypto_rc4_blob) · [crypto_autoteste](#crypto_autoteste) · [aes_cbc_cifrar](#aes_cbc_cifrar) · [aes_cbc_cifrar_raw](#aes_cbc_cifrar_raw) · [aes_cbc_decifrar](#aes_cbc_decifrar) · [aes_cbc_decifrar_raw](#aes_cbc_decifrar_raw) · [aes_chave_objeto](#aes_chave_objeto) · [aes_iv](#aes_iv) · [aes_valores_r6](#aes_valores_r6) · [aes_verificar_r6](#aes_verificar_r6) · [aes_autoteste](#aes_autoteste)  
...
Codifica o conteúdo e devolve a matriz de módulos com a máscara de menor penalidade já aplicada. Não desenha nada: quem desenha é o AddQRCode do PL_FPDF. O critério da referência não é "produz um símbolo", é "um leitor decodifica".
...
Devolve o padrão de barras como texto de '0' e '1', um caractere por módulo. Não desenha: quem desenha é o AddBarcode do PL_FPDF.
...
Descomprime um fluxo zlib. O teto existe porque stream de terceiro é entrada NÃO CONFIÁVEL: alguns KB podem virar gigabytes (zip bomb) e derrubar a sessão.
...
| Código | Quando |
|--------|--------|
| `ORA-20890` | fluxo truncado |
| `ORA-20891` | dados DEFLATE malformados |
| `ORA-20892` | cabeçalho zlib inválido |
| `ORA-20893` | a saída passou do teto |
| `ORA-20894` | tabela interna inconsistente |
...
Comprime num fluxo zlib: um bloco com Huffman FIXA e LZ77 guloso, com escape para bloco armazenado quando não compensa — a saída nunca fica maior que a entrada mais o custo do bloco.
...
Os dois dígitos hexadecimais de um byte. Existe para remontar RAW sem passar por CHR(n), que não devolve um byte e sim o caractere daquele ponto de código — em AL32UTF8, todo valor de 128 a 255 sai com DOIS bytes.
...
MD5 de um RAW. O PDF exige MD5 no algoritmo de chave até o revisionamento 4; não serve para nada que dependa de resistência a colisão.
...
Cifra ou decifra um RAW com RC4 — a mesma operação nos dois sentidos, por ser cifra de fluxo. É o que o PDF usa nos revisionamentos 2 e 3.
...
Confere MD5 e RC4 contra vetores públicos e levanta se divergir. Vale rodar depois de instalar: uma tabela transcrita errada compila e só aparece no arquivo que o leitor recusa.

#### Sintaxe

```sql
PROCEDURE PL_FPDF_UTIL.crypto_autoteste;
```
...
Cifra um BLOB em AES-CBC com preenchimento PKCS#5, como o PDF pede. O tamanho da chave decide entre AES-128 e AES-256.
...
RAW - o resultado, com o IV na frente
...
Decifra um BLOB cifrado em AES-CBC, tomando os primeiros 16 bytes como IV e removendo o preenchimento.
...
Deriva a chave de um objeto a partir da chave do documento e da numeração dele, como manda o algoritmo 1 do PDF. Cada objeto é cifrado com uma chave própria: é isso que impede trocar um objeto de lugar.
...
Monta as entradas /U, /UE, /O, /OE e /Perms do dicionário de criptografia do revisionamento 6 (AES-256, PDF 2.0), que é onde a senha passa por SHA-256 endurecido em vez de MD5.
...
Confere a senha contra as entradas do dicionário e, acertando, devolve a chave do documento e diz se era a senha de dono.
...
BOOLEAN - TRUE se a senha confere
...
```sql
PROCEDURE PL_FPDF_UTIL.aes_autoteste;
```
